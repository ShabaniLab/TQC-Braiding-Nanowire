...
def nanowire_network_graph(G, pos_par, pos_volt, states, seq):
    ...
    def update(index):
        ...
        title = None
        if index is 0:
            title = "TQC Braiding Nanowire - 1 Qubit Hadamard"
        index += 1
    ...
    ani.save('hadamard-nanowire-table.gif', writer='imagemagick')
    # plt.show()
...
def braid_particle_positions(sequence,positions):
    ...
    def update(index,ax,braid_table,positions,time,braid1,braid2,braid3,braid4):
        ...
        if index==pos_n-1:
            ...
            ax2.yaxis.set_ticks(np.arange(1, 2*par_n, 3.499*2/3))
        ...
        title = None
        if index <= 1:
            title = 'TQC - Hadamard Braid Pattern'
        else:
    ...
    ani.save('hadamard-braid-table.gif', writer='imagemagick')
    # plt.show()
