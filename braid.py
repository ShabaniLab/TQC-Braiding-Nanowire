...
def get_empty_positions(nanowire,intersection):
    ...
    if len(temp)>0:
        positions.extend(temp)
    if len(positions)==0:
        msg = "No Empty Branches on Intersection {}".format((nanowire.index(intersection)+1))
...
def get_1st_pair_sequence(pair,nanowire,intermediate_positions,final_positions):
    return list(reversed(list(pair)))

def get_2nd_pair_sequence(pair,nanowire,intermediate_positions,final_positions):
    # return list(reversed(list(pair)))
    return pair
...
def braid_particles(nanowire, vertices, matrix, sequence, positions):
    try:
        for pair in sequence:
            ...
            msg = "Error while braiding {}: {} is an invalid state".format(pair,final_positions)
            ...
            pair = get_1st_pair_sequence(pair, nanowire, positions, final_positions)
            for par in pair:
            ...
            p_pair = get_2nd_pair_sequence(pair,nanowire,intermediate_positions,final_positions)
            for par in p_pair:
                pos_start = intermediate_positions[par-1]
                pos_end = final_positions[par-1]
                path = graph.route(matrix,pos_start,pos_end)
                update_particle_movements(par,path,v11,v12,v21,v22)
